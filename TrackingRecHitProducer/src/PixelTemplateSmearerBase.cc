    ...
    TrackingRecHitAlgorithm(name,config,consumesCollector)
{
  ...
  rhitsRZfull->GetXaxis()->SetTitle("Z [cm]");
  rhitsRZfull->GetYaxis()->SetTitle("R [cm]");
  rhitsRZ = fs->make<TH2F>("newrechitsZPerp","",600,-60,60,600,-60,60);
  rhitsRZ->GetXaxis()->SetTitle("Z [cm]");
  rhitsRZ->GetYaxis()->SetTitle("R [cm]");
  rhitsXY = fs->make<TH2F>("newrechitsXY","",750,-130,130,750,-130,130);
  rhitsXY->GetXaxis()->SetTitle("X [cm]");
  rhitsXY->GetYaxis()->SetTitle("Y [cm]");
